...
class GridNeighbours(Cell):
    ...
    def get_neighbours_from_grids(self, atom_grid_idx: Tensor, num_neighbours: int):
        ...
        #     print('The number of neighour atoms in neighbour list:')
    ...
    def construct(self,
        ...
        neighours = self.get_neighbours_from_grids(
            atom_grid_idx, self.num_neighbours)

        mask = neighours != self.num_atoms
        atom_idx = msnp.broadcast_to(self.atom_idx, neighours.shape)
        neighours = F.select(mask, neighours, atom_idx)
        mask = (neighours != atom_idx)
