...
class Params:
    ...
    def __init__(self, atom_types, force_constants, atom_names=None):
    ...
    def get_bond_params(self, bonds, atom_types):
        ...
        mask_id = np.where(
            bond_types.reshape(bonds.shape[-2] * 2, 1) == self._bonds["name"]
        )

        # Checking
        # for i in range(bond_types.shape[1]):
        #     if not np.isin(bond_types[0][i], self._bonds['name']) and not np.isin(bond_types[1][i], self._bonds['name']):
        #         print (bond_types[0][i])
    ...
    def get_angle_params(self, angles, atom_types):
        ...
        mask_id = np.where(
            angle_types.reshape(angles.shape[0] * 2, 1) == self._angles["name"]
        )
    ...
    def get_dihedral_params(self, dihedrals_in, atom_types, return_includes=False):
        ...
        mask_id = np.where(
            dihedral_types.reshape(
                dihedrals.shape[0] * 2, 1) == self._dihedrals["name"]
        )

        # if mask_id[0].shape[0] < dihedrals.shape[0]:
        #     raise ValueError("Elements in atom types not recognized!")
        ...
        include_1 = np.pad(
            ...
            ((0, self._dihedrals["ks"].shape[1] - 1), (0, 0)),
            mode="edge",
        ).T.flatten()[:, None]
        ...
        dihedral_params = np.pad(
            dihedrals[:, None, :],
            ((0, 0), (0, self._dihedrals["ks"].shape[1] - 1), (0, 0)),
            mode="edge",
        ).reshape(dihedrals.shape[0] * 4, self._dihedrals["ks"].shape[1])
        dihedral_params = np.append(
            dihedral_params,
            self._dihedrals["periodicitys"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        dihedral_params = np.append(
            dihedral_params,
            self._dihedrals["ks"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        dihedral_params = np.append(
            dihedral_params,
            self._dihedrals["phi0s"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        ...
        dihedral_params_1 = np.pad(
            standar_dihedrals[:, None, :],
            ((0, 0), (0, self._dihedrals["ks"].shape[1] - 1), (0, 0)),
            mode="edge",
        ).reshape(dihedrals.shape[0] * 4, self._dihedrals["ks"].shape[1])
        dihedral_params_1 = np.append(
            dihedral_params_1,
            self._dihedrals["periodicitys"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        ...
        dihedral_params_3 = np.pad(
            standar_dihedrals[:, None, :],
            ((0, 0), (0, self._dihedrals["ks"].shape[1] - 1), (0, 0)),
            mode="edge",
        ).reshape(dihedrals.shape[0] * 4, self._dihedrals["ks"].shape[1])
        dihedral_params_3 = np.append(
            dihedral_params_3,
            self._dihedrals["periodicitys"][dihedral_id].flatten()[:, None],
            axis=1,
        )
    ...
    def _get_idihedral_params(self, dihedrals_in, atom_types, return_includes=False):
        ...
        dihedral_id = -1 * np.ones(dihedrals.shape[0], dtype=np.int32)
        mask_id = np.where(
            dihedral_types.reshape(dihedrals.shape[0] * 2, 1)
            == self._idihedrals["name"]
        )
        ...
        dihedral_params = np.append(
            dihedral_params,
            self._idihedrals["k"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        dihedral_params = np.append(
            dihedral_params,
            self._idihedrals["phi0"][dihedral_id].flatten()[:, None],
            axis=1,
        )
        ...
        dihedral_params_3 *= (
            include_2[:, None] * exclude_1[:, None] * exclude_4[:, None]
        )
    ...
    def get_vdw_params(self, atom_names):
        ...
        for i in range(self.atom_nums):
            this_id = np.where(
                np.isin(self.vdw_params["name"], atom_names[i]))[0]
            if atom_names[i] in ["N2", "NA", "NB"]:
                this_id = [24]
            if atom_names[i] in ["CA", "CC", "CR", "CW", "CN", "CB", "CV"]:
                this_id = [17]
            self.vdw_param[i][0] = self.vdw_params["radius"][this_id]
            self.vdw_param[i][1] = self.vdw_params["well_depth"][this_id]
    ...
    def get_nb14_index(self, dihedrals, angles, bonds):
        ...
        include_index = np.where(
            np.isin(nb_hash, angle_hash) + np.isin(nb_hash, bond_hash) == 0
        )[0]
